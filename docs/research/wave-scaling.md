...
Researched 2026-08-30 against installed **Brotato v1.1.15.4**. Source ids `S1`–`S14` in this file
are `WAVE-S1`–`WAVE-S14` in `RESOURCES.md`; read the access notes there before trusting a constant.

Vocabulary follows `CONTEXT.md`: **wave**, **Danger**, **zone**, **run**.

## Wave structure

- A normal run is **20 waves**; clearing wave 20 ends the run. [S1, S13]
- **Durations:** wave 1 = 20s, then +5s per wave until it caps at 60s — so wave 2 = 25s, wave 3 =
  30s, wave 9 = 60s, and waves 9–19 are all 60s. **Wave 20 = 90s.** [S1, S13]
- **Wave 20:** one boss spawns from that zone's pool. **On Danger 5, both bosses spawn instead,
  each with −25% HP.** [S1, S2, S13]
...
- Gated by Danger: **D0–D1 none, D2–D3 one, D4–D5 three.** [S2, S3]
- Slots: **wave 11 or 12**, **wave 14 or 15**, **wave 17 or 18**. The first two roll
  **60% Elite / 40% Horde**; the third is **always Elite**. [S1, S3]
- One elite per Elite wave, and an elite cannot repeat later in the same run. Elites spawning on
  **wave 11 or 12 have only 75% HP**. [S3, S4]
- Elite kill → Legendary crate (guaranteed Tier 4 item) + 100 HP heal. [S2, S3]
- Horde waves: many extra enemies, and **all enemies drop 35% fewer materials**. [S3, S13]
- The wave *slots* are identical in the Crash Zone and the Abyss; only the elite and boss rosters
  differ (the DLC adds 9 Abyss elites and 2 Abyss bosses, Dead Whale and Eel). [S3, S7]
...
- **The rule is per-enemy, not global.** Each enemy has a base HP and a **`+hp/wave`** increment,
  ...
  it has `10 + (5 × 10) = 60`. **Damage works the same way** with its own `+dmg/wave`. [S4]
- **Speed does not scale** — "their Speed doesn't change with Wave or Danger level." [S4]
- Danger multipliers apply on top, multiplicatively with the accessibility sliders. [S2, S4]
- Aggregate per-wave difficulty, datamined at v1.1.15.3 in the Abyss: total wave HP grows roughly
  ...
  the sampled composition. [S10]
- Materials per wave scale similarly: Abyss W1 = 27, W5 = 137, W19 = 541; a run starts with 30
  materials. [S10]

## Danger 0–5

Each row is the **total** for that tier, not cumulative on top of the previous. [S2]
...
The Danger 3/4/5 attribution is contested — see open question 16 in `RESOURCES.md`.

- Danger also changes **when** enemies first appear, not just whether. Enemy tables use notation
  like `13 D1:8`, meaning first spawn at wave 13 on Danger 0 but wave 8 on Danger 1+. [S4]
- **No material or gold penalty is attached to Danger level in any source found.** The only
  documented material penalty is the −35% on Horde waves. Treat "Danger reduces materials" as
  unsupported. [S3]
- No extra environmental hazards at Danger 0–5; hazards are a Nightmare feature. [S2, S9]
- Accessibility sliders sit outside the Danger system — damage and HP 25–200%, speed 25–150% —
  multiplying with Danger. [S2]
...
- Unlocked by beating Danger 5; the official notes say **10 times across different zones or
  characters** (a secondary summary said "once" — take the patch-note wording). [S9]
- Two new wave-level event types beyond a stat bump [S9]:
...
- Zone differences (community-observed, not datamined): the Abyss has guaranteed curse-inflicting
  ...
  themed with a specific challenger per wave, while Abyss waves mix enemy types. [S10, S11]
- **No evidence of "portal waves" or "abyss waves" was found** — see open question 18.
- Per-wave HP totals differ between zones but neither is uniformly higher (Crash Zone W15 = 28,964
  ...
  Abyss**; the difficulty difference comes from enemy composition. [S4, S10]
...
- The only *measured* source found is a run tracker with 10,310 submitted runs: wave 6 = 1,125
  ...
  wave 4 = 574 (7%). **Waves 6–7 alone ≈ 24% of deaths.** [S12]
- Caveats: aggregated across all characters, zones, Dangers and modes with no version stamp, from
  self-selected tracker users. Do not treat it as a measurement of this player — the save file is
...
- Qualitative agreement: Crash Zone hurdles at waves 6–7 (Slasher Eggs, Horn Spitters), 11, and 18
  ...
  elite/horde, and 17 (Stonefish stalkers). The 6–7 spike matches the tracker. [S11]

## What this topic still doesn't know

Open questions 16–22 in `RESOURCES.md`. Most load-bearing for the mission: which Danger tier first
