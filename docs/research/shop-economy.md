...
Researched 2026-08-30 against installed **Brotato v1.1.15.4**. Source ids `S1`–`S15` in this file
are `SHOP-S1`–`SHOP-S15` in `RESOURCES.md`; read the access notes there before trusting a constant.
...
- Materials are green blobs dropped by killed enemies; base drop chance **100%**, with a
  predetermined base material value per enemy. [S2]
- **Drop-chance decay:** from wave 5 onward, drop chance is reduced by **1.5% × wave number**, down
  to a **50%** floor. Wave 5 = 92.5%, wave 10 = 85%, wave 20 = 70%. [S2, S4]
- **Horde waves apply ×0.65** to material drops (35% fewer). [S2]
- **Ground cap: 50 material blobs on the map at once.** Beyond that, new material value merges into
  an existing blob rather than spawning a new one. [S2]
- **Uncollected materials are not lost:** at wave end, materials still on the ground are "bagged",
  and on later waves one unit is extracted from the bag and added to each dropped blob. [S2, S12]
- **Unspent materials carry over** as run currency — confirmed indirectly but strongly by Piggy
  ...
  i.e. compounding interest on carried materials. [S9]
...
- Picking up materials grants **both XP and currency** (same number). Bag and Piggy Bank give
  currency only, no XP — unlike Harvesting. [S2, S9]
- DLC: cursed enemies drop **+33% materials** (was +20% before 1.1.2.0). [S7, S8]
...
- **Interest:** after the payout, Harvesting grows by **5%, rounded up** —
  ...
  Harvesting before the tick is +2. [S3]
- **Rate modifiers:** Farmer +3% (→8%, and starts with +20 Harvesting); Crown +8% (→13%, stacking
  ...
  modifications, including the interest gain. [S3]
- **Negative Harvesting** loses materials and XP at wave end (you cannot lose a level), and the 5%
  interest does not apply while negative. [S3]
- Crown and Piggy Bank were **banned from appearing after wave 20** in patch 1.1.13.0. [S7]
...
- **Recycling/selling refund:** base **25%**; Recycling Machine +35% → 60%; Entrepreneur +25%,
  ...
  it rises with inflation and *falls* if you have Coupons. [S1, S10]
...
- Wiki table (waves 1–20), first-reroll / increase: 1/1, 2/1, 3/1, 4/1, 5/2, 6/2, 7/2, 9/3, 9/3,
  ...
  16–17) look like typos but are consistent with the formula. [S1]
...
- **Buying out the shop** (all 4 items) grants a **free full reroll** — changed in 1.1.0.0 from
  merely zeroing the current reroll cost. [S1]
- History: 1.1.4.0 "rerolls are now cheaper"; 1.1.7.1 "reroll price slightly increased". [S7, S15]

## Locking

- Locking is **free** and can be toggled as often as you like. [S1]
- A locked item **survives rerolls** and **persists into the next wave's shop**. [S1]
- **A locked item's price does not change** — "locked items are not affected by inflation if you
  keep them locked between shops." Locking is a genuine price hedge, not just a reservation. [S1]
- Patch 1.1.13.0 added an **item ban system with 8 ban tokens** plus lock/ban/reroll shortcuts —
  distinct from locking. [S7]

## Weapon slots and merging

- Most characters have **6 weapon slots**. Exceptions: One Armed 1, Multitasker 12, Bull cannot use
  weapons, Baby up to 24 (one per level). [S5]
...
- Economically, merging is the cheap path to high tiers: two Tier 1 Knives (10 + 10) produce a
  ...
  **unconfirmed** — see open question 12. [S5]
...
- Stated formula: `((Chance per Wave × (Current Wave − Min Wave − 1)) + Base Chance) × (100% + Luck)`,
  capped at Max Chance. **Tiers are rolled one at a time starting with Tier 4.** [S1]
  This formula does **not** reproduce the table below — see open question 10.
- 0-Luck table, waves 1–10 [S1]:
...
- **Luck multiplies these by `(100% + Luck)` but can never exceed the per-tier max.** Because the
  ...
  early and largely irrelevant late.** Roughly 290 Luck maxes out Tier 4 rarity. [S1, S11]
- Luck separately affects crates:
  `Enemy_Box_Drop_Chance × (100% + Luck) / (1 + Boxes_Spawned_This_Wave)`. [S11]
...
- Shop weapons and items can spawn **cursed**: positives multiplied and downsides divided by
  ...
  1.5–2.1 at wave 20. In exchange the item grants Curse depending on its price. [S8]
- **Chance a shop item is cursed = `15 × Curse / (50 + Curse)`**, asymptotically capped at 15%.
  (Cursed-*enemy* chance caps at 50%.) [S8]
- **Fish Hook** (limit 3): locked items and weapons have a **20% chance to become cursed when
  ...
  pity system for it. [S7, S8]

## What this topic still doesn't know

Open questions 9–15 in `RESOURCES.md`. Most load-bearing: the tier-chance formula contradicting its
